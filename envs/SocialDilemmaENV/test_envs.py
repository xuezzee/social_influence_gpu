...
class cleanENV():
    def __init__(self):
        self.maptype = "cleanup"

class TestCleanupEnv(MapEnv):
    def printBeam(self):
        self.env = CleanupEnv(ascii_map=FIRING_CLEANUP_MAP, num_agents=2)
        self.env.reset()

        self.move_agent('agent-0', [2, 4])
        self.move_agent('agent-1', [4, 2])
        self.rotate_agent('agent-0', 'DOWN')
        print("orientation....",self.env.agents["agent-0"].orientation)
        print("test-map......0")
        print(self.env.test_map)
